...
public class MapTaskServant extends TaskServant {
	...
	public boolean doRun() throws IOMapReduceException { 
		try {
			this.setInitTime();
			reporter.report(1, "Tempo de inicio da task de map (ID = " + this.getId() + "): " +
					this.getInitTime());
			reporter.report(1,"MapTask: Open input - Filename: " + configFileName);
			if(inputSplit == null)
				reporter.report(1,"MapTask: inputSplit babou");
			if(inputSplit[0] == null)
				reporter.report(1,"MapTask: inputSplit[0] babou");
			if(input == null)
				reporter.report(1,"MapTask: babou");
			input.open(configFileName,inputSplit[0],reporter);
			AnyHolder key = new AnyHolder();
			AnyHolder value = new AnyHolder();
			reporter.report(1,"MapTask: Start Map");
			while(input.next(key,value))
			{
				if(this.kill == true) {
					reporter.report(1,"MapTask: Task killed!");
					collector.flush();
					collector.close();                        
					input.close();
					collector = null;
			...
			reporter.report(1,"MapTask: Map finished, closing collector");
			collector.flush();
			collector.close();                        
			input.close();
			collector = null;
